// src/main/java/labcqrssummarize/infra/EBookController.java
...
public class EBookController {
    ...
    public ResponseEntity<EBook> getEBookById(@PathVariable String ebookId) {
        ...
        return ebook.map(ResponseEntity::ok)
    ...
    public ResponseEntity<String> requestPublish(@RequestBody EBook publishRequest) {
        ...
        event.setPublicationStatus(publishRequest.getPublicationStatus());

        MessageChannel output = kafkaProcessor.outboundTopic();
        output.send(
        ...
        return ResponseEntity.ok("출간 요청 접수: " + ebook.getEbookId());
    }
    ...
    public ResponseEntity<String> getAIProcessingStatus(@PathVariable String ebookId) {
        ...
        String status = isReady ? "AI 처리 완료" : "AI 처리 중";
    ...
    public ResponseEntity<EBook> updateEBook(
        ...
        ebook.setPublicationStatus(updateRequest.getPublicationStatus());
        // 필요한 다른 필드 업데이트
